...
If the generic skill tool is unavailable for a plugin-shipped converter, resolve the installed `alex-act-document-tools` root and read the matching `skills/<format>/SKILL.md` directly. Do not declare the converter absent when its installed file exists.
